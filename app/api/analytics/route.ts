...
import { getTeamOverview } from "@/services/analytics/analytics-service";
import { obsEnd, obsError, obsStart } from "@/lib/obs/server";

export const dynamic = "force-dynamic";

export async function GET(request: Request) {
  const obs = obsStart(request, "/api/analytics");
  try {
    const session = await requireApiSession();
    obs.userId = session.userId;
    obs.teamId = session.teamId;
    const overview = await getTeamOverview(session.teamId!);
    obsEnd(obs, 200);
    return jsonOk({ overview }, { headers: { "x-request-id": obs.requestId } });
  } catch (e: unknown) {
    obsError(obs, e);
